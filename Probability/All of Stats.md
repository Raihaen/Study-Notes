...
- We say that $X_n$ converges in distribution to $X$ if : $ \lim_{n \rightarrow \inf} CDF_n(t) = CDF(t) $ for each point $t$ at which $CDF$ is **continuous**... $CDF_n$ being the Cumulative density function of $X_n$ and $CDF$ that of $X$.
...
**Useful lemma - Markov's inequality:** For non-negative random variable $X$, we have : $P(X>t) \leq \frac{E(x)}{t}$, proof is trivial.
