...
If we assume that for each dimension's data is centreted at 0 (aka $E(x_{0j}) = 0$ for all $j$), and since $x_0 x_0^T$ is a matrix of the form {$x_{0i} \cdot x_{0j}$} for all $i,j$, we can consider it of the form : {$[x_{0i} - E(x_{0i})] \cdot [x_{0j} - E(x_{0j})]$} which is the def of $Cov(x_{0i} , x_{0j})$.
