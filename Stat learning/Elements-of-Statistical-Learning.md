...
If we assume that for each dimension's data is centreted at 0 (aka $E(x_{0j}) = 0$ for all $j$), and since $x_0 x_0^T$ is a matrix of the form {$x_{0i} \cdot x_{0j}$} for all $i,j$, we can consider it of the form : {$[x_{0i} - E(x_{0i})] \cdot [x_{0j} - E(x_{0j})]$}, this makes $E_{x_0}(x_{0i} x_{0j}) = E([x_{0i} - E(x_{0i})] \cdot [x_{0j} - E(x_{0j})])$the def of $Cov(x_{0i} , x_{0j})$.\
We thus can do : $E_{x_0}(x_0 x_0^T)$ = Cov(X)$, making our equation :\
$E_{x_0} \text{EPE}(x_0) = \sigma^2 +  E_{x_0} \text{Trace} (Cov(X)Cov(X)^{-1}) \frac{\sigma^2}{N}$.\
