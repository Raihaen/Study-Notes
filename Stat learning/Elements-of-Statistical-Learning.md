...
**Overfitting** One may ask, why do we not select all possible variables to run our regression, and how does that lead to overfitting. I find that the implicit answer presented by the author to be quite satisfactory, justifying it using p-values and MSE. Adding more values will lead to lower RSS (and so higher $R^2$ on the train set) since it's fitting closer to the training points. But this doesn't necessarely mean that the new variable has a relationship to the response necessarely, this is where the emphasis on p-values come from. Additinally, consulting the MSE is important since it's a 'weighted' indicator, that compares the increase in MSS to the change in the number of predictors for introducing the new variables.
...
- A constant standard deviation $\sigma$: This can be detected again by the residual plot. In case it happens, a good idea would be to apply concave function transformations to the data ($\sqrt{X}$ or $\log{X}$ for example). Sometimes, we have a good idea of the variance of each response, (for example if we're averaging multiple uncorrelated measurements); in this case a good solution would be weighted least squares.
- No outliers, and no leverage points. Outliers are points for which $Y$ is too different than its peers, while a leverage point is one with an $X$ value too different than its peers. Outliers affect the range, while leverage points highly affect the model's coefficients. One could plot the studentized residual to check for outliers, which is computed by dividing each residual by its estimated standard error (a good cutoff is a stud residual $> 3$). For leverage points, one could calculate the leverage statistic, which is $\sum \frac{1}{n} + \frac{\textbf{sqrd distance of point to the average}}{\textbf{sqrd sum of distances to the avreage}}$. On average we get $\frac{p+1}{n}$, and so, if a point has a leverage statistic greatly superior to that, we say it's a leverage point.

- Collinearity: when two variables are highly correlated with each other, we say they're colinear. Such variables make it difficult to correctly estimate $p-$values (masking the importance of some vairables) and prediction ranges (because of the effect on $p-$values)... Furthermore, you sometimes have multiple dimensions that are not necessarily correlated with each other but are still collinear, this is called collinearity. To access multicollinearity, a good way is to compute the variance inflation factor, which is $\frac{1}{1-R^2_{X_j | X_{-j}}$ i.e. the $R^2$ of a regression of $X_j$ on all other predictors. There are two potential solutions to the collinearity problem : 1/the larger the $VIF$ the more it would be judicial to drop the variable. 2/Combine the collinear variables into one single predictor, by taking the their the standarized versions' average.

#### Comparing to K-nearest neighbors

The KNN regression works in the following way: for a point $x_0$ you define a neighborhood $N_0$ of $K$ points from the training observations and then estimate $f$ using: \[
\frac{1}{1 - R^2_{X_j \mid X_{-j}}}
\]. This regression is affected by the bias-variance trade-off and the curse of dimensionality. For small numbers of predictors (say < 3 variables), KNN performs better/slightly better than linear regression at its optimal number of points, depending on whether the relation between the response and the predictors is linear or not. However, at higher dimensions, linear regression actually performs better, even when the relationship is non linear. This is due to the curse of dimensionality making it hard to use KNN regressors. And so, we might be wiling to forgo a little bit of prediction accuracy for a predictable, loyal, reliable model (for sure) -- not mentionning the difference in coefficient numbers and its effect on performance metrics.
